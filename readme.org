...
** commentaire sur init


volumes en bind mode

adaptation en script des instructions de la doc officielle de grr

mise au point des scripts : tester directement dans le conteneur

script : is_sourced : permet de tester facilement
...
Édition hors du conteneur, test dans le conteneur : mise au point facile


pourquoi un conteneur séparé :
- pour ne pas ajouter le client mariadb au conteneur php
- pour disposer d'un conteneur facile pour faire des choses sur la base de données

** conteneur app
- installation des dépendances php et des paquetages binaires spécifiques à GRR
- définition à partir de l'environnement du script de connexion à mariadb ~connect.inc.php~
- on tourne en utilisateur 1001 : pour la sécurité

** Partie docker-compose
lien vers doc officielle, très bien faite
docker compose 2.27 [[https://docs.docker.com/compose/releases/migrate/][docker compose V2]]
*** global à toutes les commandes docker : docker inspect !
*** network : bridge par application


*** configurer une application
- dans la phase de construction d'une image : ARG, ENV
- dans la phase de run d'un conteneur : ENV, secrets, config

*** Ensuite :
- secrets car ENV visible : TP1
